...
create_strat_power_report = function(doc_template, params, df_path) {
  doc = doc_template %>% 
    str_replace_all("SAMPLE_SIZE", cond_to_string(params$sample_size)) %>%
    ...
    str_replace_all("NUM_STRAT", cond_to_string(params$num_strata)) %>%
